...
if (window.localStorage.getItem("theme") === "dark-theme") {
  ...
  changeTheme.innerHTML = lightMode;
}
...
const navigationItems = (lists, active, show) => {
  lists.forEach((item) => {
    item.addEventListener("click", () => {
      removeActiveClass(active);
      removeActiveClass(show);
      item.classList.add(active);
      let idData = item.getAttribute("id-data");
      document.querySelector(idData).classList.add(show);
    });
  });
};
navigationItems(
...
navigationItems(
  portfolioListItems,
  "menu-portfolio__item--active",
  "portfolio__banners--show"
);

// intersection observer

const observeHandler = (allSection) => {
  allSection.map((section) => {
    let sectionId = section.target.className;
    if (section.isIntersecting) {
      document
        .querySelector(`.menu__item[id-data=${sectionId}]`)
...
menuItemActive.forEach((item) => {
  item.addEventListener("click", (e) => {
    ...
    let idData = item.getAttribute("id-data");
    let sectionoffset = document.querySelector(`.${idData}`).offsetTop;
    window.scrollTo({
